...
def project_pxl_to_plane(pxls, K, D, pose, plane_axis, plane_pt):
    """
    Map image pixels to 3D points on a plane.
    - rvec is in DEGREES.
    - Frames follow OpenCV: X_cam = R * X_world + t.
    Args:
        pxls:        (N,2) iterable of (u,v) pixels
        K:           3x3 camera matrix
        D:           distortion coeffs (None/empty ok)
        pose:        {'rvec': [rx,ry,rz], 'tvec': [tx,ty,tz]}  (or tuple/list (rvec, tvec))
        plane_axis:  3-vector plane axis (normal) in world frame
        plane_pt:    3-vector point on the plane in world frame
    Returns:
        List[List[float]] of shape (N,3). Uses float('nan') for parallel rays.
    """
    uv = np.asarray(pxls, dtype=np.float64).reshape(-1, 1, 2)
    ...
    # pose: accept dict or (rvec, tvec)
    if isinstance(pose, dict):
        rvec_deg = np.asarray(pose["rvec"], dtype=np.float64).reshape(3, 1)
        tvec = np.asarray(pose["tvec"], dtype=np.float64).reshape(3, 1)
    else:
        rvec_deg = np.asarray(pose[0], dtype=np.float64).reshape(3, 1)
        tvec = np.asarray(pose[1], dtype=np.float64).reshape(3, 1)

    # Rays in camera frame (undistorted, normalized)
    pts = cv.undistortPoints(uv, K, D, P=None).reshape(-1, 2)
    d_cam = np.column_stack([pts, np.ones(len(pts), dtype=np.float64)])  # (N,3)

    # Camera extrinsics
    R, _ = cv.Rodrigues(np.radians(rvec_deg))
    t = tvec
    C = (-R.T @ t).ravel()               # camera center in world
    d_w = (R.T @ d_cam.T).T              # ray directions in world

    # Plane (normalize axis)
    n = np.asarray(plane_axis, dtype=np.float64).reshape(3)
    n /= np.linalg.norm(n)
    p0 = np.asarray(plane_pt, dtype=np.float64).reshape(3)

    # Intersect: s = ((p0 - C)·n) / (d_w·n)
    num = (p0 - C) @ n
    den = d_w @ n
    s = num / den
    s[np.isclose(den, 0.0, atol=1e-12)] = np.nan

    X = C[None, :] + s[:, None] * d_w
    return X.astype(np.float64).tolist()


def pxl_in(pxls, pxl, eps=1e-12):
    """
    pxls: [(x1,y1), (x2,y2), ...]  polygon vertices (open or closed), len>=3
    pxl:  (x,y)    query point
    Returns: 1 if inside or on boundary, else 0
    """
    P = np.asarray(pxls, float).reshape(-1, 2)
    p = np.asarray(pxl, float).reshape(2)
    n = len(P)
    if n < 3:
        return 0

    # 1) Boundary check (on edge => inside)
    for i in range(n):
        a, b = P[i], P[(i+1) % n]
        ab, ap = b - a, p - a
        cross = ab[0]*ap[1] - ab[1]*ap[0]
        if abs(cross) <= eps:  # colinear with edge
            dot = ap @ ab
            if -eps <= dot <= (ab @ ab) + eps:
                return 1

    # 2) Even-odd rule (ray to +x)
    inside = False
    x, y = p
    for i in range(n):
        xi, yi = P[i]
        xj, yj = P[(i+1) % n]
        # does the edge straddle the horizontal line at y?
        if (yi > y) != (yj > y):
            x_int = xi + (xj - xi) * (y - yi) / (yj - yi)
            if x_int > x:  # strictly to the right; boundary already handled
                inside = not inside
