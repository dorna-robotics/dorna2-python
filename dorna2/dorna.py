...
class Dorna(WS):
    ...
    **kwargs: Other parameters associated to the play
    ...
    """
    continuous chains: drive through a list of waypoints as queued
    cont=1 motions, each section with its own vaj and corner radius.
    The chain starts FROM THE CURRENT POSITION to points[0] — the
    current position is NOT included in points. The last section sends
    ...
    def cjmove(self, joints, vajs, corners, **kwargs):
        ...
        for k, (p, vaj) in enumerate(zip(joints, vajs)):
            last = k == len(joints) - 1
            extra = {"cont": 0} if last else {"cont": 1, "corner": corners[k]}
            self.jmove(
                joint=list(p),
    ...
    def home_with_stop(self, index=7, val=0, dir=-1, travel=4000, trigger_signal=1, pid=[0, 0, 0, 100, 100], vaj_forward=[5, 1000, 5000], vaj_backward=[10, 1000, 5000], timeout=100, **kwargs):
        # home flag
        home_done = False
